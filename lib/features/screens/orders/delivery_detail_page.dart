...
class _DeliveryDetailPageState extends State<DeliveryDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        backgroundColor: isDark ? AppColors.cardBg(context) : AppColors.white,
        elevation: 0,
              ...
              : SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 16),
                      _buildStatusBanner(
                          statusBg, primary, textPrimary, isDark),
                      const SizedBox(height: 16),
                      _buildStatisticsRow(surfaceColor, borderColor,
                          textPrimary, textMuted, isDark),
                      const SizedBox(height: 16),
                      _buildVehicleCard(surfaceColor, borderColor, primary,
                          textPrimary, textMuted, isDark),
                      const SizedBox(height: 16),
                      _buildBioCard(surfaceColor, borderColor, textPrimary,
                          textMuted, isDark),
  ...
  Widget _buildProfileCard(
    ...
    final name = _agent?['name']?.toString() ??
        widget.deliveryName ??
        'Repartidor asignado';
      ...
      child: Column(
        children: [
          Stack(
            ...
            children: [
              _buildProfileAvatar(
                  name, photoUrl, primary, surfaceColor, isDark),
          ...
          Row(
            ...
            children: [
              ...
              Builder(
                builder: (context) {
                  final n = reviewsCount is num
                      ? reviewsCount.round()
                      : (int.tryParse(reviewsCount?.toString() ?? '0') ?? 0);
                  return Text(
                    n > 0
                        ? ' (${_formatNumber(n)} reseñas)'
                        : ' (Sin reseñas aún)',
                    style: GoogleFonts.plusJakartaSans(
  ...
  Widget _buildStatusBanner(
      Color statusBg, Color primary, Color textPrimary, bool isDark) {
    ...
    final hasCustomerLoc = customerLoc is Map &&
  ...
  Widget _buildStatisticsRow(Color surfaceColor, Color borderColor,
      Color textPrimary, Color textMuted, bool isDark) {
    ...
    return Row(
      children: [
        _statCard(
            ...
            borderColor,
            'Entregas',
            deliveries != null ? _formatNumber(deliveries) : '—',
            textPrimary,
            textMuted,
            isDark),
        const SizedBox(width: 12),
  ...
  Widget _statCard(Color surfaceColor, Color borderColor, String label,
      String value, Color textPrimary, Color textMuted, bool isDark) {
    return Expanded(
      child: Container(
  ...
  Widget _buildVehicleCard(Color surfaceColor, Color borderColor, Color primary,
      Color textPrimary, Color textMuted, bool isDark) {
  ...
  Widget _buildBioCard(Color surfaceColor, Color borderColor, Color textPrimary,
      Color textMuted, bool isDark) {
    final name =
        _agent?['name']?.toString() ?? widget.deliveryName ?? 'Repartidor';
    final firstName = name.split(' ').first;
    final reviews = _agent?['reviews'];
    String displayText =
        'Tu repartidor te llevará el pedido. Cuando llegue, podrás calificarlo y dejar tu reseña.';
    ...
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        onPressed: () {
          Navigator.of(context).push(
            MaterialPageRoute<void>(
              builder: (_) => BuyerOrderChatPage(
                  orderId: widget.orderId, commerceName: commerceName),
            ),
          );
        },
  ...
  Widget _buildProfileAvatar(String name, String? photoUrl, Color primary,
  ...
  IconData _vehicleIcon(String vehicleType, Color color) {
    ...
    if (v.contains('bicycle') || v.contains('bici')) {
      return Icons.directions_bike;
    }
    if (v.contains('car') || v.contains('auto')) {
